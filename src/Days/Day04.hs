...
partA input =
  foldl (\acc (y, line) -> (
    foldl (\acc' (x, c) -> 
        ...
        in
          if c == '@' && numRolls < 4 then acc' + 1 else acc'
    ) acc (enumerate line)
  )) 0 (enumerate input)

enumerate = zip [0..]

getNeighbors grid x y = catMaybes $ map (\(x', y') ->
    grid !? (y + y') >>= (!? (x + x'))
  ) offsets
  where
    ...
    where
      (newState, n') = foldl
        (\acc (y, line) -> foldl
          (\(st, nt) (x, c) ->
            let
              -- tiny optimization to use st instead of state here
              neighbors = getNeighbors st x y
              numRolls = length $ filter ('@'==) neighbors
            in
              if c == '@' && numRolls < 4 then (removeRoll x y st, nt + 1) else (st, nt)
          )
          acc
          (enumerate line)
        )
